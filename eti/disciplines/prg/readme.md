# Програмування (Python)

[Передмова](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.jwd2eraoy330)

## Змістовий модуль 1. Особливості мови Python

### ТЕМА 1.1. Основи синтаксису та структура програм
2л		0п 	1с
1.	Використання змінних, типів даних, та операторів.
2.	Основи роботи з введенням та виведенням даних.
3.	Використання умовних конструкцій та циклів.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.f1tha6cabr76)

### ТЕМА 1.2. Робота зі структурами даних
2л		4п 	5с
1.	Списки, кортежі, множини та словники.
2.	Методи для роботи з елементами колекцій.
3.	Вкладені структури даних та їх обробка.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.yx601sk2x9rk)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.20cn8rzc9t4v)

### Тема 1.3. Функції та модульність коду
5л		5п 	6с
1.	Створення та виклик функцій.
2.	Передача аргументів та повернення результатів.
3.	Розподіл коду на модулі та використання бібліотек.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.571ysgwjlqar)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.wy6igd1vbzfe)

## Змістовий модуль 2. Алгоритми обробки даних

### ТЕМА 2.1. Обробка текстових даних
2л		0п 	1с
1.	Маніпуляції зі строками (з'єднання, розділення, пошук).
2.	Використання регулярних виразів.
3.	Читання та обробка текстових файлів.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.xd87s2nupjx2)

### ТЕМА 2.2. Робота з табличними даними
2л		4п 	5с
1.	Основи роботи з CSV-файлами.
2.	Обробка табличних структур (списки списків).
3.	Створення та збереження структурованих даних.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.bfogadyuqxei)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.3ddct97jy8fe)

### Тема 2.3. Обробка числових даних
5л		5п 	6с
1.	Робота з масивами чисел.
2.	Застосування базових математичних функцій.
3.	Розрахунки та аналіз даних.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.1xsooztnhlel)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.3o68p3ms2l99)

## Змістовий модуль 3. Оптимізаційні алгоритми

### ТЕМА 3.1. Підготовка додатків до релізу
2л		0п 	1с
1.	Лінійний та бінарний пошук.
2.	Пошук в упорядкованих структурах даних.
3.	Оцінка ефективності алгоритмів.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.gc0c4yyfc9a1)

### ТЕМА 3.2. Сортування даних
4л		4п 	1с
1.	Прості алгоритми: вибір, вставка, бульбашка.
2.	Оптимізовані алгоритми: швидке та злиття сортування.
3.	Порівняння швидкодії алгоритмів.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.djhmukjjrthm)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.nnzihvih0een)

### ТЕМА 3.3. Динамічне програмування
2л		5п 	2с
1.	Принципи динамічного програмування.
2.	Розв’язання задач із запам’ятовуванням результатів.
3.	Приклади задач: рюкзак, підпослідовності.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.cnvdprkau3dv)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.e8afxth7oskn)

### Тема 3.4. Жадібні алгоритми
1л		0п 	8с
1.	Введення в жадібний підхід.
2.	Приклади: задачі покриття, графи.
3.	Порівняння з іншими підходами.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.e8w180q5y8tp)

## Змістовий модуль 4. Кодування та шифрування

### ТЕМА 4.1. Основи кодування даних
2л		0п 	1с
1.	Представлення даних у двійковій формі.
2.	Алгоритми кодування: Хаффмана, арифметичне кодування.
3.	Оцінка ефективності кодування.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.m0arfv8ma0lk)

### ТЕМА 4.2. Шифрування даних
4л		4п 	1с
1.	Симетричні алгоритми: шифр Цезаря, Віженера.
2.	Основи асиметричних алгоритмів.
3.	Створення та застосування ключів.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.j20am6ci7gf)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.62up73ycq3wr)

### Тема 4.3. Хешування
2л		5п 	2с
1.	Основи хешування та його використання.
2.	Різниця між шифруванням та хешуванням.
3.	Застосування: збереження паролів, контроль цілісності.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.bfgin2dyux01)

[Лабораторні роботи](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.a0bsdwmsagcf)

### Тема 4.4. Стеганографія
1л		0п 	8с
1.	Приховування інформації у зображеннях та тексті.
2.	Основи цифрових водяних знаків.
3.	Практичне застосування для захисту даних.

[Лекційний матеріал](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.vixpqwlf4z82)

[Питання для самопідготовки](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.t2ksj49oqwm8)

[Джерела](https://docs.google.com/document/d/1OiSlQx-xMkJCib5Iyd-KIml1DEaF-1sMKU_ad7SR38k/edit?tab=t.0#bookmark=id.pz3ue3ooc3sp)